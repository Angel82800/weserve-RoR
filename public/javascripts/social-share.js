(function() {
    window.SocialShareButton = {
        openUrl: function(url, popup) {
            if (popup === "true") {
                return window.open(url, 'popup', 'height=480,width=640,menubar=no,status=no,location=no');
            } else {
                window.open(url);
                return false;
            }
        ...
        share: function(el) {
            var $parent, appkey, desc, get_tumblr_extra, img, popup, site, title, tumblr_params, url, via, via_str, share_url, source;
            source = 'WeServe'
            site = $(el).data('site');
            ...
            if ($parent.attr("getUrlFromLocation")) {
                share_url = encodeURIComponent($parent.data("url") + '?url_server=' + window.location.href);
            }
            else{
                share_url = url
            }
            ...
            switch (site) {
                ...
                case "twitter":
                    ...
                    SocialShareButton.openUrl("https://twitter.com/intent/tweet?url=" + share_url + "&text=" + title + via_str, popup);
                    break;
                case "douban":
                    SocialShareButton.openUrl("http://shuo.douban.com/!service/share?href=" + url + "&name=" + title + "&image=" + img + "&sel=" + desc, popup);
                    break;
                case "facebook":
                    SocialShareButton.openUrl("http://www.facebook.com/sharer.php?u=" + share_url, popup);
                    break;
                case "qq":
                ...
                case "renren":
                    ...
                    break;
                case "google_plus":
                    SocialShareButton.openUrl("https://plus.google.com/share?url=" + share_url, popup);
                    break;
                ...
                case "pinterest":
                    ...
                    break;
                case "linkedin":
                    SocialShareButton.openUrl("https://www.linkedin.com/shareArticle?mini=true&url=" + url + "&title=" + title + "&summary=" + desc + "&source=" + source, popup)
                    break;
                case "tumblr":
                    get_tumblr_extra = function(param) {
                        var custom_data;
                        custom_data = $(el).attr("data-" + param);
                        if (custom_data) {
                            return encodeURIComponent(custom_data);
                        }
                    };
